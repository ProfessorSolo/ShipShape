...
<NoteBox title="DNS at Work">
  Docker Compose runs a tiny DNS server inside the project network. 
  
  When the API asks for `db_service`, this server gives it the internal IP of the database container.
</NoteBox>
...
<CautionBox title="Localhost vs Service Names">
  Inside a container, `localhost` still means "this container." 
  
  If the API tries to connect to `localhost:27017`, it will fail because it's looking for MongoDB inside itself. Use service names for any container-to-container traffic.
</CautionBox>
