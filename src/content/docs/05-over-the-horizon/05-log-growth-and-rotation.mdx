...
<img
  src="/assets/lessons/05-over-the-horizon/log-growth-rotation-timeline.png"
  alt="A clean technical diagram showing a massive red output.log file being sliced by a cron job into small daily compressed cyan blocks."
  width="1600"
  loading="lazy"
/>
_Fig 1. Rotation solves accumulation by turning monolithic unstructured text into organized daily archives._
...
// Only rotate if the log file actually exists
if (fs.existsSync(logPath)) {
  const dateStr = new Date().toISOString().split('T')[0]; // e.g. 2026-04-09
  const archivePath = path.join(__dirname, `app-${dateStr}.log`);

  // Copy the active log to the archive file
  fs.copyFileSync(logPath, archivePath);

  // Truncate (empty) the active log file so it starts fresh
  fs.truncateSync(logPath, 0);

  console.log(`[SYSTEM] Rotated logs to ${archivePath}`);
}
```

Once you have daily rotation working, a common next step is compression. You would not necessarily need a new tool—the same script (or a separate weekly task) could easily be expanded to take last week's daily archived logs and compress them into `.zip` or `.gz` batches to radically reduce disk usage.
